...
def get_bt_dist(tiles):
    ...
    for t in tiles:
        #print(t)
        #print(int(t[0]), int(t[2]), int(t[1]), int(t[3]))
...
def store_sel_tiles(ct_sel, store_path, n):
    ...
    #print(len(ct_sel))
    #if len(ct_sel)>0:
        #print(np.reshape(ct_sel, (len(ct_sel), 8)))
...
def create_matrix_D(vp, n, store_path, cached_tiles, ena_store):
    ...
    if len(ct_sel) > 0:
        ...
        for ct_ind, c_t in enumerate(ct_sel):
            ...
            #print('2    ', final_bt)
            #print('soma')
            #print(ct_cover + final_bt)
            #print('bt_ct overlatp', bt_ct_overlap)
            if len(bt_ct_overlap[0]) > 0:
                ...
                #print('PROVA')
                #print(bt_ct_overlap[0])
                #print(bt_ct_overlap[0].astype(str))
                #print(df_A.dtypes)
                #print(df_A.r[8:9])
                #print(r[0:2].astype(float))
                #print(df_A.r[8:9].isin(r[0:2].astype(float)))
