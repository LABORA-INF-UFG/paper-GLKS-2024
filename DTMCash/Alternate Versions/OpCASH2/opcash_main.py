...
def record_solution(batch, total_bits_internet, total_bits_wireless, Num_BT_in, Num_BT_wi):
    if batch == 0:
        with open('../../output/op2.csv', 'w') as file:
        ...
        with open('../../output/op2.csv', 'a') as file:
            file.write(str(batch) + ';' + str(total_bits_internet) + ';' + str(total_bits_wireless) + ';' + str(int(Num_BT_in)) + ';' + str(int(Num_BT_wi)) + '\n')


def record_cache(total):
    with open('../../output/cache_op2.txt', 'a') as file:
    ...
    for batch, users in enumerate(random_sets[rand_set]):
        ...
        for chunk in users:
            ...
            for user in users[chunk]:
                ...
                dt_tiles_for_user = pd.read_csv(user_path).values[:, :-1]
                dt_tiles_for_user = remove_repeating_tiles(dt_tiles_for_user)

                DT_tiles = list(DT_tiles) + list(pd.read_csv(user_path).values[:, :-1])

                print('###### USER ', user, 'VIEW POINT')
                ind_area = calculate_intersection({user:dt_tiles_for_user})
                #print(common_area)
                DRAW_AREA(ind_area, users[chunk])

            total_area = calculate_intersection({batch:DT_tiles})
            #print(total_area)
            distinct_tiles_for_user_i = {}
            total_tiles = repartition_given_polygon.repartition_tiles(total_area, vid, batch, 1, batch)
            print('###### UNION OF ALL VIEW POINTS')
            DRAW_AREA(total_area, users[chunk])


            # remove any repeating tiles if there is
            DT_tiles = remove_repeating_tiles(DT_tiles)
            
            index_of_requisited_BTs[chunk] = find_index_for_all_BTs(DT_tiles)
            #ttr = DT_tiles
            #print('test\n', DT_tiles)
            ...
            if batch == 0:
                ...
                tiles_to_be_cached = list(total_tiles)

                for user in users[chunk]:
                    SOLUTION_CS[user] = list(total_tiles)
                    SOLUTION_CACHE[user] = list(total_tiles)
